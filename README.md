...
This repository contains a high-performance implementation of **Regularised Matrix Factorisation** using **Alternating Least Squares (ALS)**, specifically optimised for the MovieLens 32M dataset. The system addresses the computational challenges of scaling collaborative filtering to 32 million interactions by utilising hardware-aware systems engineering, transitioning from a standard interpreted Python baseline to a JIT-compiled parallel engine.

## Repository Structure

To ensure the successful execution of the provided notebook, the data files and project structure must be organised as follows:
...
* **Numba**: Just-In-Time (JIT) compilation and LLVM-backed parallelisation.
...
3.  **Execute the Notebook**: Launch `MovieLens_32M_HighPerformance_ALS.ipynb`. The code is structured sequentially to allow for reproducibility:
    * **Seeding**: A global seed (42) is set at the start to ensure consistent latent initialisation and data splitting.
    * **Data Ingestion**: Section 3 utilises vectorised mapping to convert raw IDs into zero-indexed contiguous arrays.
...
ALS requires row-wise access for user updates and column-wise access for item updates. To eliminate $O(N)$ pointer-chasing overhead during column slicing, the system maintains synchronised, read-only views:
...
The implementation utilises Numba to translate Python logic into native machine code at runtime, bypassing the Global Interpreter Lock (GIL).



* **LLVM-Driven Vectorisation**: Enables SIMD execution for dot products and Ridge Regression solves.
...
To mitigate noise in sparse interaction data (the "Long-Tail" problem), the system utilises a **Hierarchical Latent Factor** framework. Item latent vectors are regularised toward a genre-informed prior, the average embedding of the item's associated genres, rather than the origin ($\mathbf{0}$). This "Bayesian Shrinkage" stabilises the latent manifold for items with low interaction counts.



## Qualitative Analysis

The model recovers meaningful semantic clusters autonomously. Analysis of the $K=2$ projection reveals clear thematic organisation along the latent axes:
...
The learned embeddings and calculated residuals serve as a generalised foundation for system integrity:
